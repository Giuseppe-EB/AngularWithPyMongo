...
from config import client, DBNAME
from app import app
...
def create_user():
    ...
    try:
        # Create new users
        try:
            print(request.get_data(True, True))
        ...
        print(record_created.inserted_id)
...
def join_auction():
    ...
    try:
        ...
        user_found = collection.find_one({ "username": cred[0], "password": cred[1]})

        print(dumps(user_found))

        if user_found :
            # Prepare the response
            print("ENTR")
            #print(dumps(auction))
            return jsonify(str(user_found['_id']))
        else:
            # Resource Not found
            return "", 404
        print("FINE")
...
def remove_user(user_id):
    ...
    try:
        # Delete the user
        delete_user = collection.delete_one({"_id": int(user_id)})
